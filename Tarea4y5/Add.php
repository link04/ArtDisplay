        ...
        if($id > 0){
         ...
         $val =$_FILES['name'];
            ...
            if($image == ""){
                ...
                ?><script>window.addEventListener("load", function(evt) {M.toast({html: 'Obra Editada!'})})</script><?
            }else{
...
/////////////Insertar
            ...
            if(!$_FILES['image']['name']){
                echo("<script> alert('La obra es un campo obligatorio'); </script>");
      ...
      mysqli_stmt_execute($stmt);
      ?><script>window.addEventListener("load", function(evt) {M.toast({html: 'Obra Agregada!'})})</script><?
  	...
  	if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
        ...
        if(isset($_GET['editar'])){
            ...
            if( $values['estado'] == 'a'){
                ...
                ?><script>window.addEventListener("load", function(evt) {M.toast({html: 'Obra Desactivada!'})})</script><?

            }else{
                conexion::ejecutar("UPDATE `obras` SET `estado`= 'a'  WHERE id = '{$id}'");
                ?><script>window.addEventListener("load", function(evt) {M.toast({html: 'Obra Activada!'})})</script><?
    ...
    <div class="container forms">
                <form  method="POST"  action="Add.php"  enctype="multipart/form-data">
                            ...
                            <div id="formDiv" class="input-field" >
                                ...
                                <input name="nombre" id="input_text" type="text" value="<?php echo $values['nombre'];?>" data-length="10" required autocomplete="off" >
                                <label for="input_text">Nombre</label>
                            </div>
                            <div id="formDiv" class="input-field ">
                                <i class="material-icons prefix">description</i>
                                <textarea name="descripcion" id="textarea2" class="materialize-textarea"  data-length="120" required><?php echo $values['descripcion'];?></textarea>
        ...
        <div  class="elPoder2"> 
            <div  class=" card hoverable waves-effect waves-light modal-trigger" > 
                    <div    onclick="buenDisplay('images/<?php echo "{$info['image']}"?>','<?php echo "{$info['nombre']}"?>','<?php echo "{$info['descripcion']}"?>');"  class="card-image waves-block waves-light">
                        <img id="laImgD"  src="mthumb.php?src=/web/Tarea4y5/images/<?php echo "{$info['image']}"?>&w=230&h=110" >
                    </div>
                    <div  class="elNameAdd card-content">
                        <span class=" grey-text text-darken-4"><?php echo "{$info['nombre']}"?></span><br>
