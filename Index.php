    ...
    $_SESSION["name"];
        ...
        if($_POST){
            ...
            $id = $_POST['id']+0;
                   ...
                   if($entro==true){
                    echo("<script>window.addEventListener('load', function(evt) {M.toast({html: 'Bienvenido {$_SESSION['name']}!'})})</script>");
                   }else if( $entro==false){
                    echo("<script>window.addEventListener('load', function(evt) {M.toast({html: 'Credenciales Incorrectas!'})})</script>");
                   }
...
///paginacion
      ...
      $page = min($pages, filter_input(INPUT_GET, 'page', FILTER_VALIDATE_INT, array(
       ...
       // The "back" link
        ...
        $_SESSION['cierreDeSession'] = $_SESSION['inicioDeSesion'] +  $t1;
         ...
         <div  class="elPoder2"> 
            <div  class=" card hoverable waves-effect waves-light modal-trigger" > 
                    <div    onclick="buenDisplay('images/<?php echo "{$info['image']}" ?>','<?php echo "{$info['nombre']}" ?>','<?php echo "{$info['descripcion']}" ?>');"  class="card-image waves-block waves-light">
                        <img id="laImgD"  src="mthumb.php?src=/images/<?php echo "{$info['image']}"?>&w=230&h=180" >
                    </div>
